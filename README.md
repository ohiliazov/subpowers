...
A workflow for coding agents that enforces absolute discipline and refuses to skip steps: **explore → spec → plan →
implement → debug → check**. Six distinct rituals, absolute constraints, and a hard line between "I think this works"
...
## The Six Rituals (Skills)

| Skill                 | The Absolute Rule                                          | Invoke when...                                                           |
|:----------------------|:-----------------------------------------------------------|:-------------------------------------------------------------------------|
...
| `subpowers-implement` | Red → green → refactor, followed by strict self-review.    | 1–2 files, purpose already absolute.                                     |
| `subpowers-debug`     | Expose the root cause before striking. 3-Strike limit.     | A test fails or behavior is wrong and you don't know why.                |
| `subpowers-check`     | Run the command, read the output, *then* claim victory.    | Before saying "done" — and automatically after a plan's last task.       |
...
| Section                              | How the rituals use it                                                                                          |
|:-------------------------------------|:----------------------------------------------------------------------------------------------------------------|
...
| `## Project rules`                   | The condensed, checkable form of your `CLAUDE.md` — applied strictly during the Implement ritual's self-review. |
